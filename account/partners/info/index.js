    ...
    let coinIssuanceTable = `
      <table class="table table-borderless table-coin-issuance">
        ...
        .join("")}
              ...
              <div>
                <button class="btn btn-outline-dark" onclick="g_data.editCoinAuditRule('${data.data.partnerData.no}')">編輯</button>
              </div>
            </div>
          </th>
        </tr>
...
let currentWalletCoin = null;
let walletModalInstance = null;

g_data.walletControl = function (walletID) {
  const row = $(`a[onclick="g_data.walletControl('${walletID}')"]`).closest('tr');
  currentWalletCoin = row.find('td:first').text().trim(); // 幣種
  currentWalletId = walletID;

  $("#walletAmountInput").val("");
  $("#walletActionType").val("");
  $("#walletActionError").hide();

  walletModalInstance = new bootstrap.Modal(document.getElementById("walletActionModal"));
  walletModalInstance.show();

  $('#walletActionModal').removeClass('show').hide();
  $('.modal-backdrop').remove();
  $('body').removeClass('modal-open').css({ 'overflow': '', 'padding-right': '' });
};

$("#walletActionConfirmBtn").on("click", function () {
  const amount = $("#walletAmountInput").val().trim();
  const action_type = $("#walletActionType").val();

  if (!action_type) {
    $("#walletActionError").text("請選擇操作類型").show();
    return;
  }

  if (!amount || isNaN(amount)) {
    $("#walletActionError").text("請輸入正確金額").show();
    return;
  }

  $.post("/api/partners/wallet_action.php", {
    wallet_id: currentWalletId,
    amount: amount,
    action_type: action_type,
    coin: currentWalletCoin
  }, function (res) {
    if (res.success) {
      ...
      $("#walletActionError").text(res.message || "操作失敗").show();
    }
  });
});

let coinAuditModalInstance = null;

let coinAuditRules = {}; // 暫存從 API 載入的幣別條件

g_data.editCoinAuditRule = function (partnerId) {
  $("#coinAuditPartnerId").val(partnerId);
  $("#coinAuditAmountLimit").val("");
  $("#coinAuditDailyLimit").val("");
  $("#coinAuditError").hide();
  $("#coinAuditCoin").val("USDT");
  $("#coinAuditCoinText").text("USDT");

  coinAuditModalInstance = new bootstrap.Modal(document.getElementById("coinAuditModal"));
  coinAuditModalInstance.show();
  ...
  // 讀取規則
  $.get("/api/partners/get_coin_audit_rules.php", { partner_id: partnerId }, function (res) {
    if (res.success) {
      coinAuditRules = {};
      res.data.forEach(rule => {
        coinAuditRules[rule.coin] = rule;
      });

      const currentCoin = $("#coinAuditCoin").val();
      if (coinAuditRules[currentCoin]) {
        $("#coinAuditAmountLimit").val(coinAuditRules[currentCoin].amount_limit);
        $("#coinAuditDailyLimit").val(coinAuditRules[currentCoin].daily_limit);
      }
    }
  });
};

$("#coinAuditCoin").on("change", function () {
  const selectedCoin = $(this).val();
  $("#coinAuditCoinText").text(selectedCoin);
  if (coinAuditRules[selectedCoin]) {
    $("#coinAuditAmountLimit").val(coinAuditRules[selectedCoin].amount_limit);
    $("#coinAuditDailyLimit").val(coinAuditRules[selectedCoin].daily_limit);
  } else {
    $("#coinAuditAmountLimit").val("");
    $("#coinAuditDailyLimit").val("");
  }
...
$("#coinAuditSaveBtn").on("click", function () {
  const partner_id = $("#coinAuditPartnerId").val();
  const coin = $("#coinAuditCoin").val();
  const amount_limit = $("#coinAuditAmountLimit").val();
  const daily_limit = $("#coinAuditDailyLimit").val();

  if (!amount_limit || isNaN(amount_limit) || !daily_limit || isNaN(daily_limit)) {
    $("#coinAuditError").text("請正確輸入所有欄位").show();
    return;
  }

  $.post("/api/partners/update_coin_audit_rules.php", {
    partner_id,
    coin,
    amount_limit,
    daily_limit
    ...
    if (res.success) {
      coinAuditModalInstance.hide();
      //alert("儲存成功");
      //location.reload();

      // 成功後即時更新顯示區塊（不重整頁面）
        if (coinAuditRules[coin]) {
          coinAuditRules[coin].amount_limit = amount_limit;
          coinAuditRules[coin].daily_limit = daily_limit;
        }

        if ($("#coinIssuanceSelect").val() === coin) {
          $(".table-coin-issuance-th1").text("數量條件(每筆): " + amount_limit);
          $(".table-coin-issuance-th2").text("超過 " + amount_limit + " " + coin);
          $(".table-coin-issuance-th3").text("筆數條件(每日)﹡: " + daily_limit);
          $(".table-coin-issuance-th4").text("超過 1 筆");
        }

    } else {
      $("#coinAuditError").text(res.message || "儲存失敗").show();
    }
  });
});
...
g_data.renderCoinIssuanceRules = function (partnerId) {
  $.get("/api/partners/get_coin_audit_rules.php", { partner_id: partnerId }, function (res) {
    if (res.success) {
      const ruleMap = {};
      res.data.forEach(rule => {
        ruleMap[rule.coin] = rule;
      });

      function updateFields(coin) {
        const rule = ruleMap[coin];
        if (rule) {
          $(".table-coin-issuance-th1").text("數量條件(每筆): " + rule.amount_limit);
          $(".table-coin-issuance-th2").text("超過 " + rule.amount_limit + " " + coin);
          $(".table-coin-issuance-th3").text("筆數條件(每日)﹡: " + rule.daily_limit);
          $(".table-coin-issuance-th4").text("超過 1 筆");
        } else {
          $(".table-coin-issuance-th1").text("數量條件(每筆): -");
          $(".table-coin-issuance-th2").text("超過 -");
          $(".table-coin-issuance-th3").text("筆數條件(每日)﹡: -");
          $(".table-coin-issuance-th4").text("超過 -");
        }
      }

      $("#coinIssuanceSelect").off("change").on("change", function () {
        const coin = $(this).find("option:selected").text();
        updateFields(coin);
      });

      const defaultCoin = $("#coinIssuanceSelect").find("option:selected").text();
      updateFields(defaultCoin);
    }
  });
};
