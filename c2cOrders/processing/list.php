...
try {
    // 總筆數
    $countStmt = $pdo->prepare("SELECT COUNT(*) FROM partner_c2c_orders_processing $where");
    $countStmt->execute($params);
    $totalCount = $countStmt->fetchColumn();
    $totalPage = ceil($totalCount / $limit);

    // 分頁資料
    $stmt = $pdo->prepare("SELECT * FROM partner_c2c_orders_processing $where ORDER BY create_time DESC LIMIT :offset, :limit");
