...
function requestData() {
  ...
  let cb = function (data) {
    ...
    const statusMap = {
      1: { text: "挂单中", class: "info" },
      2: { text: "付款中", class: "secondary" },
      3: { text: "已完成", class: "success" },
      4: { text: "已取消", class: "warning" },
    }
        ...
        ${data
          ...
          <tr>
            ...
            <td>
              ...
              <td><a href="javascript:void(0)" class="text-primary" onclick="g_data.viewUserDetail('${item.seller}')">${item.seller}</a></td>
              <td>${item.currency}</td>
              <td>${item.amount}</td>
              <td>${item.min_deal_amount}</td>
              <td>${item.deal_amount}</td>
              <td>${item.dealing_amount || "-"}</td>
              <td>${item.remain_amount || "-"}</td>
              <td>${item.refunded_amount || "-"}</td>
              <td>${item.expect_fee}</td>
              <td>${item.actual_fee || "-"}</td>
              <td>${item.agent_profit || "-"}</td>
              <td>${item.create_time || "-"}</td>
              <td>${item.secomplete_timeller || "-"}</td>
            </tr>
          `;
            })
            .join("")}
...
g_data.viewOrderDetail = function (orderId) {
  ...
  // 跳转到订单详情页
  let hash = "/c2cOrders/sell/info/?id=" + orderId;
