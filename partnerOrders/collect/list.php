...
try {
    $countStmt = $pdo->prepare("SELECT COUNT(*) FROM partner_collect_orders $where");
    $countStmt->execute($params);
    $totalCount = $countStmt->fetchColumn();
    $totalPage = ceil($totalCount / $limit);

    $stmt = $pdo->prepare("SELECT * FROM partner_collect_orders $where ORDER BY time DESC LIMIT :offset, :limit");
