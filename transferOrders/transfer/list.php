...
if ($keyword !== '') {
    switch ($orderType) {
        ...
        case '2': $where .= " AND sender LIKE :keyword"; break;
        case '3': $where .= " AND receiver LIKE :keyword"; break;
    }
...
try {
    $countStmt = $pdo->prepare("SELECT COUNT(*) FROM partner_transfer_orders $where");
    $countStmt->execute($params);
    $totalCount = $countStmt->fetchColumn();
    $totalPage = ceil($totalCount / $limit);

    $stmt = $pdo->prepare("SELECT * FROM partner_transfer_orders $where ORDER BY createTime DESC LIMIT :offset, :limit");
